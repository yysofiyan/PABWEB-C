...
class Mahasiswa extends CI_Controller
{
    ...
    public function tambah_data()
    {
        ...
        $label_prodi = form_label('Pogram Studi', 'prodi');
        $attr_id = array(
              'type' => 'hidden',
              'name' => 'id_mahasiswa',
    ...
    {
        $this->rules();
        $validasi = $this->form_validation_>run();
        if ($validasi == FALSE) {
            $this->tambah_data();
        } else {
            ...
            $this->session->set_flashdata('pesa','Data berhasil ditambah!');
            redirect('mahasiswa');
        }
    }
    public function edit($id)
    {
        ...
        if ($get_row->num_rows() > 0) {
            ...
            $form_submit = form_submit('sumbit', 'simpan');
            $error_nim = form_error('nim');
            $error_nama = form_error('nama_mhs');
            $data = array(
                'form_open' => $form_open,
    ...
    public function edit_aksi()
    {
        ...
        $this->session->set_flashtdata('pesan', 'Data berhasil diubah!');
        redirect('mahasiswa');
        }
    }
