<?php
// panggil file databse.php untuk koneksi ke database
...
if (isset($_POST['simpan'])){
    ...
    if(empty($nama_file)){
        // perintah wuery untuk mengubah data pada tabel siswa
 ...
 $update = mysqli_query($db, "UPDATE tbl_siswa SET nama ='$nama',
  ...
  foto ='$nama_file'
  WHERE nis='$nis'")
 or die('ada kesalahan pada query query : '.mysqli_error($db));
        // cek query    
        if($update) {
    // jika berhasil tampilkan pesan berhasil ubah data
    header("location: index.php?alert=2");
